...
def main(cfg: DictConfig):
    ...
    if cfg.debug.wandb:
        ...
        end_dt = datetime.datetime(
            ...
            cfg.end_epoch.hour,
            cfg.end_epoch.minute,
            int(cfg.end_epoch.second),          # integer seconds
            int((cfg.end_epoch.second % 1) * 1e6)  # microseconds
        )
        for i in range(366):

            new_date = end_dt + timedelta(days=i)
            ...
            run.log({"gs_list_lat": gs_list_plot[0],
                    ...
                    "num_days": i,
                    "mean_contact_num": len(contacts_exclusion_secs) /(i+1),
